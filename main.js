...
function createChartConfig({ labels, data, label, type, colors, }) {
    return {
        ...
        options: {
            animation: {
                duration: 0, // Disable animations for static PDF rendering
            },
        },
    };
}
async function main() {
    ...
    const { text: marketTrends, sources } = await (0, ai_1.generateText)({
        model: (0, google_1.google)("gemini-1.5-flash"),
        tools: {
            google_search: google_1.google.tools.googleSearch({}),
        },
        prompt: `Search the web for market trends for plant-based milk in North America for 2024. I need to know the market size, key players and their market share, and primary consumer drivers. `,
    });
    console.log("Market trends found.");
    // Step 2: Extract chart data
    const { object: chartData } = await (0, ai_1.generateObject)({
        model: (0, google_1.google)("gemini-1.5-flash"),
        schema: zod_1.z.object({
            chartConfigurations: zod_1.z
                .array(zod_1.z.object({
                type: zod_1.z.enum(["bar", "line"]).describe('The type of chart to generate. Either "bar" or "line"'),
                labels: zod_1.z.array(zod_1.z.string()).describe("A list of chart labels"),
                data: zod_1.z.array(zod_1.z.number()).describe("A list of the chart data"),
                label: zod_1.z.string().describe("A label for the chart"),
                colors: zod_1.z.array(zod_1.z.string()).describe('A list of colors to use for the chart, e.g. "rgba(255, 99, 132, 0.8)"'),
            }))
                .describe("A list of chart configurations"),
        }),
        prompt: `Given the following market trends text, come up with a list of 1-3 meaningful bar or line charts and generate chart data. Market Trends: ${marketTrends} `,
    });
    ...
    const { text: htmlReport } = await (0, ai_1.generateText)({
        model: (0, google_1.google)("gemini-1.5-flash"),
        prompt: `You are an expert financial analyst and report writer. Your task is to generate a comprehensive market analysis report in HTML format.
...
Include this script in the head of the HTML:
